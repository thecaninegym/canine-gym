  ...
  useEffect(() => {
    const fetchDogs = async () => {
      const { data } = await supabase
        .from('dogs')
        .select('*, owners(name, email, phone), leaderboard_settings(city)')
        .order('name')
      setDogs(data || [])
      setLoading(false)
    }
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      ...
      <div style={{ padding: '32px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          ...
          <a href="/admin/dogs/new" style={{ backgroundColor: '#FF6B35', color: 'white', padding: '10px 20px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold' }}>+ Add Dog</a>
        </div>
        {loading ? (
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            {dogs.map(dog => (
              <div key={dog.id} style={{ backgroundColor: 'white', padding: '24px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <a href={`/admin/sessions/new?dog=${dog.id}`} style={{ backgroundColor: '#FF6B35', color: 'white', padding: '10px 20px', borderRadius: '6px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px' }}>
                  Log Session
                </a>
              </div>
            ))}
