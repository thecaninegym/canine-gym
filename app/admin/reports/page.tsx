...
export default function AdminReports() {
  ...
  const alaCarteRevenue = currentBookings.filter(b => b.status === 'confirmed' || b.status === 'completed').length * 50
  ...
  const formatPeriodLabel = () => {
    if (period === 'week') {
      return `${current.start.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${current.end.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`
    }
  ...
  const ChangeTag = ({ value }: { value: number }) => (
    <span style={{
      backgroundColor: value > 0 ? '#d4edda' : value < 0 ? '#f8d7da' : '#f0f0f0',
      color: value > 0 ? '#155724' : value < 0 ? '#721c24' : '#666',
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f5' }}>
      ...
      <div style={{ padding: '32px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
          {[
            { label: 'Bookings', value: currentBookings.length, change: bookingChange, prev: previousBookings.length, icon: '📅' },
            { label: 'Sessions Logged', value: currentSessions.length, change: sessionChange, prev: previousSessions.length, icon: '🏃' },
            { label: 'New Clients', value: currentNewClients.length, change: clientChange, prev: previousNewClients.length, icon: '👤' },
            { label: 'Active Members', value: activeMemberships.length, change: null, prev: null, icon: '💳' },
            { label: 'Total Clients', value: owners.length, change: null, prev: null, icon: '🐾' },
            { label: 'Monthly MRR', value: `$${membershipRevenue.toLocaleString()}`, change: null, prev: null, icon: '💰' },
          ].map((metric, i) => (
            <div key={i} style={{ backgroundColor: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
              <p style={{ margin: '0 0 4px 0', fontSize: '13px', color: '#666', textTransform: 'uppercase', fontWeight: 'bold' }}>{metric.icon} {metric.label}</p>
              ...
              {metric.prev !== null && metric.change === null && (
                <p style={{ margin: 0, fontSize: '12px', color: '#999' }}>prev {period}: {metric.prev}</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: '24px' }}>
            <h3 style={{ color: '#003087', margin: '0 0 20px 0' }}>📍 Bookings by City</h3>
            {citySorted.length === 0 ? (
          ...
          {/* Bookings by day of week */}
          <div style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: '24px' }}>
            <h3 style={{ color: '#003087', margin: '0 0 20px 0' }}>📆 Bookings by Day</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: '24px' }}>
            <h3 style={{ color: '#003087', margin: '0 0 20px 0' }}>💳 Membership Breakdown</h3>
            {[
          ...
          {/* Longest clients */}
          <div style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: '24px' }}>
            <h3 style={{ color: '#003087', margin: '0 0 20px 0' }}>🏆 Longest Clients</h3>
        ...
        {/* Cancellation rate */}
        <div style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', padding: '24px' }}>
          <h3 style={{ color: '#003087', margin: '0 0 20px 0' }}>📉 Booking Health</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px' }}>
            {[
              { label: 'Confirmed', value: filterByPeriod(bookings, 'booking_date', current).filter(b => b.status === 'confirmed').length, color: '#003087' },
              { label: 'Completed', value: filterByPeriod(bookings, 'booking_date', current).filter(b => b.status === 'completed').length, color: '#28a745' },
              { label: 'Cancelled', value: filterByPeriod(bookings, 'booking_date', current).filter(b => b.status === 'cancelled').length, color: '#dc3545' },
              { label: 'No Shows', value: filterByPeriod(bookings, 'booking_date', current).filter(b => b.status === 'no_show').length, color: '#ffc107' },
