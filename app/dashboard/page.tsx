...
export default function ClientDashboard() {
  ...
  const fetchFriendsActivity = async (myOwnerId: string) => {
    const { data: followsData } = await supabase
      .from('follows')
      .select('following_owner_id')
      .eq('follower_owner_id', myOwnerId)
    ...
    const { data: friendDogs } = await supabase
      .from('dogs')
      .select('id, name, photo_url, owner_id, owners(name)')
    ...
    const { data: recentSessions } = await supabase
      .from('sessions')
      .select('id, dog_id, session_date, distance_miles, calories_burned, duration_minutes')
